// ceci est la version de base du projet avec les fonctions que j'ai rajoutées (Ici, il faut un Servo Moteur pour que l'user se rende compte qu'il a reçu quelque chose)
...
void setup() {
  ...
  // Initialize TFT
  tft.begin();
  tft.setRotation(1); // Change orientation to horizontal
  ...
  tft.setCursor(0, 0);
  tft.println("");
  tft.print("La boite est prete a etre utilisee");
  delay(5000);
  tft.fillScreen(TFT_BLACK);
  tft.setRotation(0); // Change orientation to horizontal
  bot.sendMessage(chat_id, "Configuration terminée");
}
...
void getUserInputFromServer() {
  ...
  HTTPClient http;
  String serverAddress = "your server adress";  // Adresse de mon serveur Flask
 ...
 Serial.print(user_input);
  ...
  tft.setCursor(10, 10);
  tft.println("  ");
  tft.println(user_input);
  tft.setRotation(0); // Réinitialiser la rotation à la valeur par défaut
...
void new_string() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Spécifier l'URL
    http.begin("tonsiteweb.com/delete_user_input");
...
void loop() {
  ...
  if(afficher_message) {
    ...
    Serial.print(user_input);
    Serial.print("\n");
    bot.sendMessage(chat_id, "Tu as reçu un message, tu peux le voir ici :le lien vers le message");
    new_string(); 
    user_input = "";
...
case WAITING_TO_DISPLAY_PNG:
  if (analogRead(LIGHT_SENSE_PIN) < 5) {
    ...
    if (millis() - startTime > 60000) {  // Vérifie si 30 secondes se sont écoulées
      bot.sendMessage(chat_id, "Tu as reçu une image, tu peux la voir ici : le lien vers l'image");
      isTiming = false;  // Réinitialise le chronométrage
      showImage();
      currState = WAITING_FOR_IMAGE;
      ledcWrite(0, pulseWidth(MID_POS));
